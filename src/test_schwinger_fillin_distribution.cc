 ...
 *
 * @brief Main program for testing the fill-in distribution for the quenched
 * Schwinger model
 *
...
void save_to_disk(const double beta, const double phi_12, const double phi_23,
                  const double phi_34, const double phi_41,
                  const std::vector<double> theta, const std::string filename) {
  FILE *datafile;
 ...
 * @param[in] phi_41 Perimeter angle \f$\phi_{41}\f$
 */
void generate_gaussian_samples(const double beta, const unsigned int n_samples,
  ...
  for (unsigned int k = 0; k < n_samples; ++k) {
    gaussian_fillin_dist.draw(engine, mod_2pi(phi_12), mod_2pi(phi_23),
                              mod_2pi(phi_34), mod_2pi(phi_41),
                              theta[4 * k + 0], theta[4 * k + 1],
                              theta[4 * k + 2], theta[4 * k + 3]);
  }
  save_to_disk(beta, phi_12, phi_23, phi_34, phi_41, theta,
 ...
 * @param[in] phi_41 Perimeter angle \f$\phi_{41}\f$
 */
void generate_samples(const double beta, const unsigned int n_samples,
                      const double phi_12, const double phi_23,
                      const double phi_34, const double phi_41) {
  std::mt19937_64 engine;
  engine.seed(215517);
