...
/* Evaluate QoI */
const double QoISusceptibility::evaluate(const std::shared_ptr<SampleState> x_path) {
    if (x_path->data.size() != M_lat) {
        mpi_parallel::cout << "ERROR: Evaluating QoISusceptibility on path of wrong size." << std::endl;
