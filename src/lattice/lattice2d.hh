 ...
 *
 * This stores the number \f$M_{t,lat}\f$, \f$M_{x,lat}\f$ of lattice sites in
 * the temporal and spatial direction, as well as the type of coarsening to be
 * used.\f$
 */
...
public:
  ...
  Lattice2DParameters()
      : Parameters("lattice"), Mt_lat_(8), Mx_lat_(8),
  ...
  unsigned int Mt_lat() const { return Mt_lat_; }

  /** @brief Return number of lattice points in spatial direction  */
  unsigned int Mx_lat() const { return Mx_lat_; }

  /** @brief Return coarsening type */
  CoarseningType coarsening_type() const { return coarsening_type_; }
...
public:
   ...
   *
   * Given a lattice site \f$n=(i,j)\f$, work out the corresponding linear index
   * \f$\ell\f$.
   *
   ...
   *  0-----+-----1-----+----(0)
   *
   * In other words, first the indices are distributed over the lattice where
   * both i and j are even, followed by the vertices where both i and j are odd.
   *
   ...
   *   2. a list of all vertices that are NOT present on the next-coarser
   * lattice
  ...
  inline unsigned int vertex_cart2lin(const int i, const int j) const {
    if (rotated) {
      ...
      return Mt_lat_half * (j_shifted % Mx_lat_half) + i_shifted % Mt_lat_half +
             offset;
    } else {
   ...
   *
   * Given the linear indexCoarsen \f$\ell\f$, work out cartesian index
   \f$(i,j)\f$.
   ...
   * @param[out] j Position index in spatial direction
   */
  inline void diag_vertex_lin2cart(const unsigned int ell, int &i,
                                   int &j) const {
    /* Rotated indices are only allowed if the lattice contains an
     * even number of cells in both directions
     */
   ...
   */
  inline unsigned int link_cart2lin(const int i, const int j,
                                    const int mu) const {
    // Currently links can only be handled on non-rotated lattices
    assert(not rotated);
    return 2 * Mt_lat * ((j + Mx_lat) % Mx_lat) + 2 * ((i + Mt_lat) % Mt_lat) +
           mu;
  }

  /** @brief Convert linear index of link to lattice index
   *
   * Given \f$\ell = (2 j+\mu) M_{t,lat} + i\f$, work out cartesian index
   \f$(i,j)\f$
   ...
   */
  inline void link_lin2cart(const unsigned int ell, int &i, int &j,
                            int &mu) const {
    // Currently links can only be handled on non-rotated lattices
    assert(not rotated);
...
protected:
  ...
  /** @brief map of coarse vertices to the corrresponding vertex on coarse
   * lattice */
