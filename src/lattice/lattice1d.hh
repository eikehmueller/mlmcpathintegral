...
public:
  ...
  Lattice1DParameters() : Parameters("lattice"), M_lat_(8), T_final_(1.0) {
...
public:
   ...
   */
  Lattice1D(const unsigned int M_lat_, const double T_final_,
  ...
  virtual std::shared_ptr<Lattice1D> coarse_lattice() {
    if (M_lat % 2) {
      mpi_parallel::cerr << "ERROR: cannot coarsen 1d lattice with M = "
                         << M_lat << " points." << std::endl;
      mpi_exit(EXIT_FAILURE);
      throw std::runtime_error("...");
    }
    return std::make_shared<Lattice1D>(M_lat / 2, T_final,
                                       coarsening_level + 1);
  };
