...
/* constructor */
Lattice2D::Lattice2D(const unsigned int Mt_lat_, const unsigned int Mx_lat_,
                     const CoarseningType coarsening_type_,
                     const int coarsening_level_)
    : Lattice(coarsening_level_, 2), Mt_lat(Mt_lat_), Mx_lat(Mx_lat_),
  ...
  if ((rotated) and ((Mx_lat % 2) or (Mt_lat % 2))) {
    mpi_parallel::cerr
        << "ERROR: Both Mx_lat and Mt_lat have to be even for rotated lattices."
        << std::endl;
  ...
  coarsening_allowed =
      coarsening_allowed and (Mt_lat_coarse > 1) and (Mx_lat_coarse > 1);
  if (coarsening_allowed) {
    coarse_lattice = std::make_shared<Lattice2D>(
        Mt_lat_coarse, Mx_lat_coarse, coarsening_type, coarsening_level + 1);
    ...
    for (auto it = coarse_vertices.begin(); it < coarse_vertices.end(); ++it) {
      ...
      unsigned int ell_coarse =
          coarse_lattice->vertex_cart2lin(i / rho_coarsen_t, j / rho_coarsen_x);
