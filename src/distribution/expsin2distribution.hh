 ...
 *
 * @brief Class for sampling from the distribution \f$p(x) =
 * Z_{\sigma}^{-1}\exp\left[-\sigma \sin^2\left(\frac{x}{2}\right)\right]\f$
 * with \f$x\in[-\pi,\pi]\f$
 *
...
public:
   ...
   */
  ExpSin2Distribution()
      : distribution(0.0, 1.0), normal_distribution(0.0, 1.0) {}
  ...
  const double draw(URNG &engine, const double sigma) const {
    ...
    while (true) {
      ...
      if (fabs(r_x) < M_PI) {
        ...
        if (r_accept < exp(-sigma * (sin_psi_half * sin_psi_half -
                                     r_x * r_x / (M_PI * M_PI)))) {
          return r_x;
        }
      }
...
private:
   ...
   * Compute \f$2\pi e^{-z}I_0(z)\f$, using a Taylor approximation for large
   * values of z
   *
