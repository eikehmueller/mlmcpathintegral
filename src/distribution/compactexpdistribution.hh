 ...
 *
 * @brief Class for sampling from the distribution \f$p_\sigma(x) =
 * Z_{\sigma}^{-1}\exp\left[\sigma x\right]\f$ with \f$x\in[-1,+1]\f$
 *
 * The normalisation constant is
 * \f$Z_{\sigma}=\frac{e^\sigma-e^{-\sigma}}{\sigma}\f$ and the cumulative
 * distribution is \f$F_{\sigma}(x) = \frac{e^{\sigma
 * x}-e^{-\sigma}}{e^{\sigma}-e^{-\sigma}}\f$.
 *
 *
 * To sample from this distribution we use the inverse transform method
 * (see e.g. https://en.wikipedia.org/wiki/Inverse_transform_sampling). For
 * this, we draw a number \f$u\f$ uniformly from the interval \f$[0,1)\f$ and
 * compute
 *
 * \f[
