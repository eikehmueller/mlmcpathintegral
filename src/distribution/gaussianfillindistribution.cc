...
double GaussianFillinDistribution::evaluate(
    const double theta_1, const double theta_2, const double theta_3,
    const double theta_4, const double phi_12, const double phi_23,
    const double phi_34, const double phi_41) const {
  double eta_1 = mod_2pi(0.5 * (theta_1 + theta_2 - theta_3 - theta_4) +
                         0.5 * (phi_41 - phi_23));
  double eta_2 = mod_2pi(0.5 * (theta_1 - theta_2 - theta_3 + theta_4) +
                         0.5 * (phi_34 - phi_12));
  double eta_3 = mod_2pi(0.5 * (theta_1 - theta_2 + theta_3 - theta_4) +
                         0.25 * (-phi_12 + phi_23 - phi_34 + phi_41));
...
void GaussianFillinDistribution::construct_peaks(int n_offsets) {
  ...
  std::array<std::array<int, 3>, 9> p_main = {
      0, 0, 0, 2,  2,  2, -2, 2, 2,  2,  -2, 2,  -2, -2,
      2, 2, 2, -2, -2, 2, -2, 2, -2, -2, -2, -2, -2};
  std::array<std::array<int, 3>, 4> p_secondary = {2, 0, 1,  -2, 0,  1,
                                                   0, 2, -1, 0,  -2, -1};
    ...
    for (int k_y = -n_offsets; k_y <= +n_offsets; ++k_y) {
      for (int k_z = -n_offsets; k_z <= +n_offsets; ++k_z) {
        ...
        auto add_offset = [&](std::array<int, 3> a) {
          return std::array<int, 3>{a[0] + 4 * k_x, a[1] + 4 * k_y,
                                    a[2] + 4 * k_z};
        };
