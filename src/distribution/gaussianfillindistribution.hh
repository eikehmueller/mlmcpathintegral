 ...
 *
 * @brief Header file for preconditioned 4D Gaussian distribution used for
 * fill-in in quenched Schwinger model
 */

/** @class GaussianFillinDistribution
 *
 * @brief Class for sampling from a Gaussian approximation of the fill-in
 * distribution
 *
 * During fill-in for the quenched Schwinger model we need to sample from the
 * following distribution:
 *
 * \f[
 * p_0(\theta|\phi) \propto \exp\left[ \beta \left(
 * \cos(\theta_1-\theta_2-\phi_{12})
 *                                                 +
 * \cos(\theta_2-\theta_3-\phi_{23})
 *                                                 +
 * \cos(\theta_3-\theta_4-\phi_{34})
 *                                                 +
 * \cos(\theta_4-\theta_1-\phi_{41}) \right) \right] \f] for given values of
 * \f$\phi\f$. This class approximates this distribution by a multivariate
 * Gaussian.
 *
 * If the parameter add_gaussian_noise is set to false, the two Gaussian
 * distributions are further replaced by delta-functions, in other words only
 * one of the most likely configurations is returned.
 */
...
public:
   ...
   * @param[in] add_gaussian_noise_ Add Gaussian noise? If false, only take peak
   * value
   */
  GaussianFillinDistribution(const double beta_,
                             const bool add_gaussian_noise_ = false)
      : beta(beta_), add_gaussian_noise(add_gaussian_noise_),
        uniform_distribution(0.0, 1.0), normal_distribution(0.0, 1.0),
        sqrt2(sqrt(2.0)) {
    if (not add_gaussian_noise) {
      mpi_parallel::cerr << "ERROR: sampling only from peak currently broken."
                         << std::endl;
  ...
  /** @brief Draw number directly from Gaussian distribution for given
   * \f$\phi\f$
   *
   ...
   */
  template <class URNG>
  void draw(URNG &engine, const double phi_12, const double phi_23,
            const double phi_34, const double phi_41, double &theta_1,
    ...
    theta_3 = mod_2pi(0.5 * (-eta_1 - eta_2 + eta_3) + omega + 2. * Phi -
                      phi_12 - phi_23);
    theta_4 = mod_2pi(0.5 * (-eta_1 + eta_2 - eta_3) + omega + 3. * Phi -
                      phi_12 - phi_23 - phi_34);
  }
  ...
  double evaluate(const double theta_1, const double theta_2,
                  ...
                  const double phi_12, const double phi_23, const double phi_34,
                  const double phi_41) const;
