...
public:
  ...
  BesselProductDistribution(double beta_) :
    ...
    sigma_beta(M_PI/sqrt(2*log(I0_twobeta))) {
        /* Compute expansion coefficients of normalisation constant */
  ...
  template <class URNG>
    const double draw(URNG& engine, const double x_p, const double x_m) const {
        ...
        double N_m = gsl_sf_erf(0.5*dx/sigma_beta)*pow(I0_twobeta,2.*(1.-dx/M_PI));
        double C_gauss = pow(I0_twobeta,2.*(1.-dx*dx/(M_PI*M_PI)));
