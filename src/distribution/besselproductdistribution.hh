 ...
 * @brief Header file for distribution given by the product of two modified
 * Bessel functions
 */
 ...
 * Z_{x_+,x_-}^{-1}I_0(2\beta \cos((x-x_+)/2))I_0(2\beta \cos((x-x_-)/2))\f$
 *
 ...
 * able to sample from \f$p(x|y,0)\f$ with \f$y\ge 0\f$. In this case it is
 * possible to construct two separate Gaussian envelopes for the two intervals
 * \f$[-\pi,-\pi+y]\f$ and \f$[-\pi+y,+\pi]\f$. To generate a sample, first work
 * out which of those two intervals the envelope sample is draw from (using the
 * appropriate integrals over the envelope distribution), then draw a sample
 * from the envelope distribution and finally accept/reject with the appropriate
 * ratio.
 *
 * Note that the normalisation constant of the distribution \f$p(x|x_+,x_-)\f$
 * can be written as a Fourier- cosine expansion of the angle \f$x_+-x_-\f$,
 * where the expansion coefficients depend on the value of the parameter
 * \f$\beta\f$. Some care needs to be taken when truncating this sum, the code
 * currently only reliably supports value of \f$\beta\f$ not exceeding 8. For
 * larger values, kmax and nmax would need to be adjusted.
 */
...
public:
  ...
  BesselProductDistribution(double beta_)
      : beta(beta_), distribution(0.0, 1.0), normal_distribution(0.0, 1.0),
        kmax(16), nmax(32), I0_twobeta(gsl_sf_bessel_I0(2 * beta)),
    ...
    for (unsigned int k = 0; k <= kmax; ++k) {
      ...
      for (unsigned int n = k; n <= nmax; ++n) {
        for (unsigned int m = k; m <= nmax; ++m) {
          double log_comb = log_nCk(2 * n, n - k) + log_nCk(2 * m, m - k) -
                            2 * (log_factorial(n) + log_factorial(m));
  ...
  const double draw(URNG &engine, const double x_p, const double x_m) const {
    ...
    double sign_flip =
        (dx < 0) ? -1 : +1; // Flip sign if difference is negative
    dx *= sign_flip;
    // Work out (scaled) normalisation constants of both sides of the
    // envelope density
    double N_p = gsl_sf_erf((M_PI - 0.5 * dx) / sigma_beta);
    double N_m = gsl_sf_erf(0.5 * dx / sigma_beta) *
                 pow(I0_twobeta, 2. * (dx / M_PI - 1.));
    double C_gauss_p =
        pow(I0_twobeta, 2. * (1. - dx * dx / (4. * M_PI * M_PI)));
    double C_gauss_m =
        pow(I0_twobeta, 2. * (1. - (dx - 2. * M_PI) * (dx - 2. * M_PI) /
                                       (4. * M_PI * M_PI)));
