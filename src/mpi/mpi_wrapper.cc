...
bool mpi_master() { return (mpi_comm_rank() == 0); }
...
std::vector<double> mpi_allreduce_avg(const std::vector<double> x) {
  ...
  MPI_Allreduce(send_data, recv_data, n_data, MPI::DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
...
#ifdef USE_MPI
  MPI_Scatter(data, 1, MPI::UNSIGNED, &x, 1, MPI::UNSIGNED, 0, MPI_COMM_WORLD);
#else
  x = data[0];
#endif // USE_MPI
}
