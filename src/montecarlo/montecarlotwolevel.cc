 ...
 */
/* Constructor */
MonteCarloTwoLevel::MonteCarloTwoLevel(
    ...
    : MonteCarlo(param_twolevelmc.n_burnin()),
      n_samples(param_twolevelmc.n_samples()), fine_action(fine_action_),
      qoi_fine(qoi_factory_->get(fine_action_)),
      qoi_coarse(qoi_factory_->get(fine_action_->coarse_action())),
      t_indep(0.0), n_indep(0), t_sampler(0),
      ...
      stats_coarse_sampler("QoI[coarsesampler]",
                           param_stats.n_autocorr_window()) {
  mpi_parallel::cout << "Twolevel Monte Carlo:" << std::endl;
  coarse_action = fine_action->coarse_action();
  mpi_parallel::cout << "  fine action   : " << fine_action->info_string()
                     << std::endl;
  mpi_parallel::cout << "  coarse action : " << coarse_action->info_string()
                     << std::endl;
  coarse_sampler = sampler_factory->get(coarse_action);
  conditioned_fine_action = conditioned_fine_action_factory->get(fine_action);
  twolevel_step = std::make_shared<TwoLevelMetropolisStep>(
      coarse_action, fine_action, conditioned_fine_action);
}
...
void MonteCarloTwoLevel::draw_coarse_sample(
    std::shared_ptr<SampleState> phi_state) {
