...
public:
  ...
  TwoLevelMCParameters()
      : Parameters("twolevelmc"), n_burnin_(100), n_samples_(100),
        n_coarse_autocorr_window_(10), n_fine_autocorr_window_(10),
        n_delta_autocorr_window_(10), sampler_(SamplerHMC) {
  ...
  int readFile(const std::string filename) {
    ...
    if (!readSuccess) {
      ...
      n_coarse_autocorr_window_ =
          getContents("n_coarse_autocorr_window")->getInt();
      n_fine_autocorr_window_ = getContents("n_fine_autocorr_window")->getInt();
      n_delta_autocorr_window_ =
          getContents("n_delta_autocorr_window")->getInt();
      ...
      if (sampler_str == "HMC") {
        ...
        mpi_parallel::cerr
            << "        allowed values are \'HMC\', \'heatbath\', \'cluster\', "
               "\'hierarchical\', \'exact\'"
            << std::endl;
...
private:
  ...
  /** @brief Size of window used for measuring coarse level sampler
   * autocorrelations */
...
public:
   ...
   * @param[in] sampler_factory Factory for constructing conditioned fine
   * actions
...
private:
  ...
  /** @brief number of skipped samples between independent coarse samples on
   * coarse levels */
  double t_indep;
