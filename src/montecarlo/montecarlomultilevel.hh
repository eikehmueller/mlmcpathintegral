...
public:
  ...
  MultiLevelMCParameters()
      : Parameters("multilevelmc"), n_level_(2), n_burnin_(100), epsilon_(1.0),
        show_detailed_stats_(false), sampler_(SamplerMultilevel) {
    ...
    if (!readSuccess) {
      ...
      if (sampler_str == "hierarchical") {
        ...
        mpi_parallel::cerr << " ERROR: Unknown sampler: " << sampler_str
                           << std::endl;
        mpi_parallel::cerr << "        allowed values are \'hierarchical\', "
                              "\'multilevel\', \'cluster\'"
                           << std::endl;
...
private:
   ...
   * Calculate
   * \f[
   *   C^{eff}_{\ell} = \lceil \tau^{int}_{\ell} \rceil (C_\ell +
   * C_{\ell+1}^{indep}) \f]
   *
...
private:
  ...
  /** @brief Timer class */
  Timer timer;
  /** @brief number of skipped samples between independent coarse samples on all
   * levels */
