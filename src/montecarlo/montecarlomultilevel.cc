...
MonteCarloMultiLevel::MonteCarloMultiLevel(
    ...
    : MonteCarlo(param_multilevelmc.n_burnin()), fine_action(fine_action_),
      ...
      n_min_samples_qoi(param_stats.n_min_samples_qoi()), timer("MultilevelMC"),
  ...
  for (unsigned int ell = 0; ell < n_level - 1; ++ell) {
    ...
    stats_coarse_sampler.push_back(std::make_shared<Statistics>(
        stats_sampler_label.str(), n_autocorr_window));
  }
  for (unsigned int ell = 0; ell < n_level; ++ell) {
    mpi_parallel::cout << "  action on level " << ell << " : "
                       << action[ell]->info_string() << std::endl;
  }
  ...
  for (unsigned int ell = 0; ell < n_level; ++ell) {
    phi_state.push_back(
        std::make_shared<SampleState>(action[ell]->sample_size()));
    phi_coarse_state.push_back(
        std::make_shared<SampleState>(action[ell]->sample_size()));
  }
  ...
  for (unsigned int level = 0; level < n_level; ++level) {
    ...
    stats_qoi.push_back(
        std::make_shared<Statistics>(stats_qoi_label.str(), n_autocorr_window));
  }
}
...
void MonteCarloMultiLevel::evaluate() {
  ...
  for (int level = n_level - 1; level >= 0; level--) {
    ...
    for (unsigned int j = 0; j < n_burnin; ++j) {
      if (level == n_level - 1) {
        ...
        twolevel_step[level]->draw(phi_coarse_state[level + 1],
                                   phi_state[level]);
        double qoi_fine = qoi[level]->evaluate(phi_state[level]);
        double qoi_coarse =
            qoi[level + 1]->evaluate(phi_coarse_state[level + 1]);
  ...
  do {
    ...
    for (unsigned int ell = 0; ell < n_level; ++ell) {
      ...
      double C_ell_eff = cost_eff(ell);
      n_target[ell] = ceil(two_epsilon_inv2 * sum_s_ell2_C_ell_eff *
                           sqrt(V_ell / C_ell_eff) * tau_int);
...
void MonteCarloMultiLevel::draw_coarse_sample(
    const unsigned int level, std::shared_ptr<SampleState> phi_state) {
  ...
  t_indep[level - 1] =
      (n_indep[level - 1] * t_indep[level - 1] + t_sampler[level - 1]) /
      (1.0 + n_indep[level - 1]);
  n_indep[level - 1]++;
...
void MonteCarloMultiLevel::show_detailed_statistics() {
  mpi_parallel::cout << "=== Statistics of coarse level samplers ==="
                     << std::endl;
  for (unsigned int level = 1; level < n_level; ++level) {
    ...
    mpi_parallel::cout << " sub-sampling rate = " << t_indep[level - 1]
                       << std::endl;
  ...
  for (unsigned int level = 0; level < n_level; ++level) {
    ...
    mpi_parallel::cout << " target number of samples = " << n_target[level]
                       << std::endl;
    mpi_parallel::cout << " cost [per indep. sample]              "
                       << cost_eff(level) << " mu s" << std::endl;
    if (level < n_level - 1) {
      mpi_parallel::cout << " cost [per sample in two-level step] "
                         << twolevel_step[level]->cost_per_sample() << " mu s"
                         << std::endl;
    }
    mpi_parallel::cout << "------------------------------------" << std::endl;
  }
  ...
  for (unsigned int level = 0; level < n_level; ++level) {
    double cost_per_level = 1.E-6 * cost_eff(level) /
                            ceil(stats_qoi[level]->tau_int()) *
                            stats_qoi[level]->samples();
    cost_level.push_back(cost_per_level);
    cost_total += cost_per_level;
  }
  for (unsigned int level = 0; level < n_level; ++level) {
    mpi_parallel::cout << " level " << level << " : " << cost_level[level]
                       << " s [ " << 100 * cost_level[level] / cost_total
                       << " ] %" << std::endl;
  }
...
void MonteCarloMultiLevel::show_statistics() {
  ...
  mpi_parallel::cout << std::endl;
  mpi_parallel::cout << " Q: Avg +/- Err = " << numerical_result() << " +/- "
                     << statistical_error() << std::endl;
  mpi_parallel::cout << std::endl;
