...
void Statistics::record_sample(const double Q) {
  ...
  avg_longterm = ((n_samples_longterm - 1.0) * avg_longterm + Q) /
                 (1.0 * n_samples_longterm);
  avg2_longterm = ((n_samples_longterm - 1.0) * avg2_longterm + Q * Q) /
                  (1.0 * n_samples_longterm);
  avg3_longterm = ((n_samples_longterm - 1.0) * avg3_longterm + Q * Q * Q) /
                  (1.0 * n_samples_longterm);
  avg4_longterm = ((n_samples_longterm - 1.0) * avg4_longterm + Q * Q * Q * Q) /
                  (1.0 * n_samples_longterm);
  // Update running S_k
...
double Statistics::variance_error() const {
  ...
  unsigned int n_samples_ = mpi_allreduce_sum(n_samples_longterm);
  return sqrt(1.0 / n_samples_ *
              (avg4_ - 4 * avg_ * avg3_ + 8 * avg_ * avg_ * avg2_ -
               avg2_ * avg2_ - 4 * avg_ * avg_ * avg_ * avg_));
}
...
unsigned int Statistics::local_samples() const { return n_samples; }
...
std::ostream &operator<<(std::ostream &os, const Statistics &stats) {
  ...
  os << " " << stats.label() << ": tau_{int}   = " << stats.tau_int()
     << std::endl;
  os << " " << stats.label() << ": window      = " << stats.autocorr_window()
     << std::endl;
  os << " " << stats.label() << ": # samples   = " << stats.samples()
     << std::endl;
  return os;
}
