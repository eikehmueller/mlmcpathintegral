...
CommandLineParser::CommandLineParser(const int argc, char *argv[]) {
  for (int j = 1; j < argc; ++j) {
    ...
    if (malformatted) {
      mpi_parallel::cout << "ERROR: malformatted command line option \'" << sraw
                         << "\'" << std::endl;
      exit(-1);
    }
    dict[key] = value;
...
bool CommandLineParser::getopt_string(const std::string key,
...
bool CommandLineParser::getopt_int(const std::string key, int &value) const {
  ...
  try {
    ...
    mpi_parallel::cout << "WARNING: could not convert \'" << value
                       << "\' to integer:" << std::endl;
...
bool CommandLineParser::getopt_ulong(const std::string key,
...
bool CommandLineParser::getopt_double(const std::string key,
                                      double &value) const {
  if (dict.count(key) == 0) {
    return false;
  }
