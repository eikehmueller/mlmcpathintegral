...
double Phi_chit(const double beta, const unsigned int n_plaq) {
  if (beta > 2000.0) {
    ...
    mpi_parallel::cerr << "ERROR: Phi_chit(beta,P) unstable for beta>2000. Use "
                          "Phi_chit_perturbative instead."
                       << std::endl;
  ...
  for (int n = 0; n < nmax; ++n) {
    phi_chit +=
        beta * weight[n] / weight_sum *
        (ddIn[n] / In[n] - (n_plaq - 1) * (dIn[n] * dIn[n]) / (In[n] * In[n]));
  }
  return phi_chit;
}
...
double Phi_chit_perturbative(const double beta, const unsigned int n_plaq) {
  ...
  double Phi_NLO = 0.5 - xi * Sigma_hat2 +
                   0.25 * xi * xi * (Sigma_hat4 - Sigma_hat2 * Sigma_hat2);
  // Total result
...
void compute_In(const double x, std::vector<double> &In,
  ...
  auto integrand_phi2 = [](double phi, void *p) -> double {
    ...
    bool include_trig = not params->use_qawo;
    double f =
        1. / (8. * M_PI * M_PI * M_PI) * phi * phi * exp(x * (cos(phi) - 1.0));
    if (include_trig)
      f *= cos(n * phi);
    return f;
  };
  ...
  if (params.use_qawo) {
    qawo_table = gsl_integration_qawo_table_alloc(1.0, 2.0 * M_PI,
                                                  GSL_INTEG_COSINE, n_level);
  }
  ...
  for (int n = 0; n < In.size(); ++n) {
    ...
    if (params.use_qawo) {
      ...
      gsl_integration_qawo(&integrand, -M_PI, epsabs, epsrel, n_workspace,
                           workspace, qawo_table, &dIn[n], &abserr);
    } else {
      gsl_integration_qag(&integrand, -M_PI, +M_PI, epsabs, epsrel, n_workspace,
                          1, workspace, &dIn[n], &abserr);
    }
    /* --- I''_n(x), weight function is cos(n*phi) --- */
    integrand.function = integrand_phi2;
    if (params.use_qawo) {
      gsl_integration_qawo_table_set(qawo_table, n, 2.0 * M_PI,
                                     GSL_INTEG_COSINE);
      gsl_integration_qawo(&integrand, -M_PI, epsabs, epsrel, n_workspace,
                           workspace, qawo_table, &ddIn[n], &abserr);
    } else {
      gsl_integration_qag(&integrand, -M_PI, +M_PI, epsabs, epsrel, n_workspace,
                          1, workspace, &ddIn[n], &abserr);
    }
  }
...
double gff_phi_squared_analytical(const double mass, const double Mt_lat,
  ...
  for (unsigned int k1 = 0; k1 < Mt_lat; ++k1) {
    for (unsigned int k2 = 0; k2 < Mx_lat; ++k2) {
      double sin_k1 = sin(M_PI * k1 / Mt_lat);
      double sin_k2 = sin(M_PI * k2 / Mx_lat);
