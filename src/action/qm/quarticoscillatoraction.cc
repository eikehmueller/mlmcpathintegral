...
/** Evaluate action */
const double QuarticOscillatorAction::evaluate(
  ...
  double S = m0 * (ainv2 * x_diff * x_diff + mu2 * x_j_squared) +
             0.5 * lambda * x_j_shifted_squared * x_j_shifted_squared;
  for (unsigned int j = 1; j < M_lat; ++j) {
    ...
    S += m0 * (ainv2 * x_diff * x_diff + mu2 * x_j_squared) +
         0.5 * lambda * x_j_shifted_squared * x_j_shifted_squared;
  }
  return 0.5 * a_lat * S;
}
...
void QuarticOscillatorAction::force(const std::shared_ptr<SampleState> x_path,
  ...
  double X_j_shifted = X_j - x0;
  p_path->data[0] =
      tmp_1 * (tmp_2 * X_j - x_path->data[M_lat - 1] - x_path->data[1]) +
      tmp_3 * X_j_shifted * X_j_shifted * X_j_shifted;
  // Interior points
  for (unsigned int j = 1; j < M_lat - 1; ++j) {
    X_j = x_path->data[j];
    X_j_shifted = X_j - x0;
    p_path->data[j] =
        tmp_1 * (tmp_2 * X_j - x_path->data[j - 1] - x_path->data[j + 1]) +
        tmp_3 * X_j_shifted * X_j_shifted * X_j_shifted;
  }
  X_j = x_path->data[M_lat - 1];
  X_j_shifted = X_j - x0;
  p_path->data[M_lat - 1] =
      tmp_1 * (tmp_2 * X_j - x_path->data[M_lat - 2] - x_path->data[0]) +
      tmp_3 * X_j_shifted * X_j_shifted * X_j_shifted;
}
