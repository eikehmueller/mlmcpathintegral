...
/** Evaluate action */
const double
RotorAction::evaluate(const std::shared_ptr<SampleState> x_path) const {
...
void RotorAction::overrelaxation_update(std::shared_ptr<SampleState> x_path,
                                        const unsigned int j) {
  double x_m;
  double x_p;
  if (j == 0) {
...
void RotorAction::initialise_state(std::shared_ptr<SampleState> x_path) const {
  ...
  std::generate(x_path->data.data(), x_path->data.data() + x_path->data.size(),
...
double RotorAction::chit_perturbative() const {
  ...
  return 1. / (4. * M_PI * M_PI * m0) *
         (1. - xi * S_hat2 +
          (0.5 - xi * S_hat2 + 0.25 * xi * xi * (S_hat4 - S_hat2 * S_hat2)) *
              z);
}
