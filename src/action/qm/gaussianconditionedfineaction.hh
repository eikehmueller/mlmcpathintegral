 ...
 * \f[
 *    S^{cond}(X;X_-,X_+) =
 * \frac{1}{2}W''(X_-,X_+)\left(X-\overline{X}(X_-,X_+)\right)^2 \f] where the
...
public:
   ...
   */
  GaussianConditionedFineAction(const std::shared_ptr<QMAction> action_)
      : action(action_) {
...
struct GaussianConditionedFineActionFactory
  ...
  get(std::shared_ptr<Action> action) {
    return std::make_shared<GaussianConditionedFineAction>(
