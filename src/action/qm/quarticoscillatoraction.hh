...
public:
  ...
  QuarticOscillatorParameters()
      : Parameters("quarticoscillator"), m0_(1.0), mu2_(1.0), lambda_(1.0),
        x0_(0.0) {
...
public:
  ...
  QuarticOscillatorAction(const std::shared_ptr<Lattice1D> lattice_,
                          ...
                          const double lambda_, const double x0_)
      : QMAction(lattice_, renormalisation_, m0_), mu2(mu2_), lambda(lambda_),
        x0(x0_) {}
  ...
  std::shared_ptr<Action> virtual coarse_action() {
    ...
    new_action = std::make_shared<QuarticOscillatorAction>(
        lattice->coarse_lattice(), renormalisation, m0, mu2, lambda, x0);
    return new_action;
  }
   ...
   * Note that for this action we have
     \f[
         P_j = \frac{m_0}{a}\left(2X_j-X_{j-1}-X_{j+1}\right) + am_0\mu^2 X_j +
   a\lambda (X_j-x_0)^3 \f]
   *
  ...
  double virtual inline getWcurvature(const double x_m,
    ...
    return (2. / a_lat + a_lat * mu2) * m0 +
           3. * lambda * a_lat * (x - x0) * (x - x0);
  }
   ...
   * action (see Action::getWminimum()) can be found as the solution of
   \f[
      x_0\left(1+\frac{1}{2}a^2\mu^2\right) + \frac{\lambda a^2}{2m_0}  x_0^3 =
   \overline{x} \f]
   * where \f$x=\frac{x_-+x_+}{2}\f$. Here, we calculate an approximate value
   * by setting \f$x_0^{(0)} = \overline{x}\f$ and then iterating
   \f[
     x_0^{(n+1)} = \left(1+\frac{1}{2}a^2\mu^2\right)^{-1}\left(\overline{x} -
   \frac{\lambda a^2}{2m_0} \left(x_0^{(n)}\right)^3\right) \f]
   * for \f$n=0,\dots,2\f$
   *
   * @param[in] x_m Value of \f$x_-\f$
   * @param[in] x_p Value of \f$x_+\f$
  ...
  double virtual inline getWminimum(const double x_m, const double x_p) const {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      x = rho * (xbar - 0.5 * a_lat * a_lat * lambda / m0 * x_shifted *
                            x_shifted * x_shifted);
    }
    return x;
  }
