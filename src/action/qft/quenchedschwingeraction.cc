...
const double QuenchedSchwingerAction::evaluate(
...
void QuenchedSchwingerAction::compute_staple_angles(
    std::shared_ptr<SampleState> phi_state, const int i, const int j,
    const int mu, double &theta_p, double &theta_m) {
  if (mu == 0) {
...
void QuenchedSchwingerAction::heatbath_update(
...
void QuenchedSchwingerAction::overrelaxation_update(
    std::shared_ptr<SampleState> phi_state, const unsigned int ell) {
  int i, j, mu;
  double theta_p;
  double theta_m;
...
void QuenchedSchwingerAction::force(
    const std::shared_ptr<SampleState> phi_state,
...
void QuenchedSchwingerAction::copy_from_coarse(
  ...
  if ((Mt_c_lat == Mt_lat / 2) and (Mx_c_lat == Mx_lat / 2)) {
    ...
    mpi_parallel::cerr << "ERROR: cannot copy from coarse lattice."
                       << std::endl;
...
void QuenchedSchwingerAction::copy_from_fine(
  ...
  if ((Mt_f_lat == 2 * Mt_lat) and (Mx_f_lat == 2 * Mx_lat)) {
    ...
    for (unsigned int i = 0; i < Mt_lat; ++i) {
      for (unsigned int j = 0; j < Mx_lat; ++j) {
        phi_state->data[lattice->link_cart2lin(i, j, 0)] = mod_2pi(
            phi_fine->data[fine_lattice->link_cart2lin(2 * i, j, 0)] +
    ...
    for (unsigned int i = 0; i < Mt_lat; ++i) {
      for (unsigned int j = 0; j < Mx_lat; ++j) {
        ...
        phi_state->data[lattice->link_cart2lin(i, j, 1)] = mod_2pi(
            phi_fine->data[fine_lattice->link_cart2lin(i, 2 * j, 1)] +
...
void QuenchedSchwingerAction::initialise_state(
