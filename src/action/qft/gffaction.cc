...
const double
GFFAction::evaluate(const std::shared_ptr<SampleState> phi_state) const {
  double S = 0.0;
  if (n_gibbs_smooth == 0) {
    // No Gibbs smoothing
    double kappa = 4. + mu2;
    const std::vector<std::vector<unsigned int>> &neighbour_vertices =
        lattice->get_neighbour_vertices();
...
void GFFAction::global_heatbath_update_eff(
  ...
  for (unsigned int ell = 0; ell < Nvertices; ++ell) {
    double Delta = (1. - omega) * (4. + 0.5 * mu2 - 4. / (4. + 0.5 * mu2)) *
                   phi_state->data[ell];
    for (int k = 0; k < 4; ++k) {
    ...
    phi_state->data[ell] =
        sigma_eff * (gamma * normal_dist(engine) + sigma_eff * Delta);
  }
}
...
GFFAction::buildPrecisionMatrix(std::vector<double> stencil) {
  unsigned int Nvertices = lattice->getNvertices();
  const std::vector<std::vector<unsigned int>> &neighbour_vertices =
      lattice->get_neighbour_vertices();
  ...
  for (unsigned int ell = 0; ell < Nvertices; ++ell) {
    ...
    for (int j = 0; j < stencil.size() - 1; ++j) {
      for (int k = 0; k < 4; ++k) {
        tripletlist[stencil_size * ell + 4 * j + k + 1] =
            T(ell, neighbour_vertices[ell][4 * j + k], stencil[j + 1]);
      }
    }
  }
...
void GFFAction::print_correlation_function() const {
  ...
  rhs[0] = 1.0;
  Eigen::VectorXd u = choleskyLT.triangularView<Eigen::Upper>().solve(
      choleskyL.triangularView<Eigen::Lower>().solve(rhs));
  std::vector<double> X;
  std::vector<double> Y;
