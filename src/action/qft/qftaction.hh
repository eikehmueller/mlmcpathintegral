...
public:
  ...
  QFTParameters()
      : Parameters("quantumfieldtheory"), action_(ActionQuenchedSchwinger) {
  ...
  QFTActionType action() const { return action_; }
...
public:
  ...
  std::shared_ptr<Lattice2D> get_lattice() { return lattice; }

  /** @brief Get underlying coarse lattice */
  std::shared_ptr<Lattice2D> get_coarse_lattice() { return coarse_lattice; }
