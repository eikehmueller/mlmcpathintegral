...
#define NONLINEARSIGMACONDITIONEDFINEACTION_HH                                 \
  NONLINEARSIGMACONDITIONEDFINEACTION_HH
 ...
 *
 * Fills in the values at the sites marked by 'X', given the values at the
 * coarse level sites marked by 'C' in the following diagrams
 *
...
public:
   ...
   */
  NonlinearSigmaConditionedFineAction(
      const std::shared_ptr<NonlinearSigmaAction> action_)
      : action(action_), beta(action->getbeta()) {
    engine.seed(115147);
  }
...
struct NonlinearSigmaConditionedFineActionFactory
  ...
  get(std::shared_ptr<Action> action) {
    std::shared_ptr<NonlinearSigmaAction> nonlinearsigma_action =
        std::dynamic_pointer_cast<NonlinearSigmaAction>(action);
    // Select correct type based on coarsening type
    return std::make_shared<NonlinearSigmaConditionedFineAction>(
        nonlinearsigma_action);
  }
};
