 ...
 *
 * Calculate the renormalised coarse grid coupling constant \f$\beta\f$ either
 * perturbatively or through non-perturbative matching. Up to
 * \f$\mathcal{O}(\beta^{-1})\f$ we have:
 *
 * \f[
...
public:
   ...
   * nonperturbative
   */
  RenormalisedQuenchedSchwingerParameters(
  ...
  double beta_coarse() {
    ...
    if (not((coarsening_type == CoarsenBoth) or
      ...
      mpi_parallel::cerr << "ERROR: invalid coarsening type in renormalisation"
                         << std::endl;
...
private:
   ...
   *
   * The perturbative value of the coarse level coupling \f$\beta^{(c)}\f$ is
   * found by matching the topological susceptibility on the fine and coarse
   * level, i.e. \f$V\chi_t(\beta^{(c)},P/4)=\chi_t(\beta,P)\f$ where \f$P\f$ is
   * the number of plaquettes on the current level, only including terms of up
   * to \f$\mathcal{O}(\beta^{-1})\f$
   */
   ...
   *
   * The non-perturbative value of the coarse level coupling \f$\beta^{(c)}\f$
   * is found by matching the topological susceptibility on the fine and coarse
   * level, i.e. \f$V\chi_t(\beta^{(c)},P/4)=V\chi_t(\beta,P)\f$ where \f$P\f$
   * is the number of plaquettes on the current level
   */
   ...
   * found by finding the zeros of the function \f$F(x;\beta,P)=
   * \\chi_t(x\beta,P/4)-\chi_t(\beta,P)\f$
   *
  ...
  static double f_root(double x, void *p) {
    ...
    return quenchedschwinger_chit_analytical(x * beta, n_plaq / rho_refine) -
           quenchedschwinger_chit_analytical(beta, n_plaq);
  };
