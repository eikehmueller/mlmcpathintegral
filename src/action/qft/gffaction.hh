 ...
 *
 * This stores the non-dimensionalised squared mass \f$\mu^2=a^2
 * m^2_{\text{cont}}\f$
 */
...
public:
  ...
  GFFParameters()
      : Parameters("gff"), mass_(1.0), renormalisation_(RenormalisationNone) {
 ...
 * \f[
 *      S[\phi] = \sum_n \frac{1}{2}\left[
 * (\phi_{n+\hat{0}-\phi_n)(\phi_{n+\hat{0}-\phi_n)
 *                                       +
 ...
 *
 * \f[
 *      S_{eff}[\phi] = \sum_n \frac{1}{2}\left[ \left(4+\mu_f^2 -
 * \frac{4}{4+\mu_f^2}\right) \phi_n^2
 *                                             -
 * \frac{2}{4+\mu_f^2}\left(\phi_{n+\hat{0}}+\phi_{n-\hat{0}}+\phi_{n+\hat{1}}+\phi_{n-\hat{1}}\right)
 * \phi_n
 *                                              -
 * \frac{1}{4+\mu_f^2}\left(\phi_{n+\hat{0}+\hat{1}}+\phi_{n+\hat{0}-\hat{1}}+\phi_{n-\hat{0}+\hat{1}}+\phi_{n-\hat{0}-\hat{1}}\right)
 * \phi_n \right]
 *
 * \f]
 *
 * with the non-dimensionalised fine-level mass
 * \f$\mu^f=\frac{1}{\sqrt{2}}\mu\f$.
 *
 * To approximately sample from this action, one can proceed as follows:
 *
 *   1. Draw a sample from the action \f$S\f$
 *   2. Apply \f$n_{Gibbs}\f$ steps of a Gibbs sampler with the stencil of
 * \f$S_{eff}\f$
 *
...
public:
  ...
  GFFAction(const std::shared_ptr<Lattice2D> lattice_,
            const std::shared_ptr<Lattice2D> fine_lattice_, const double mass_,
            const int n_gibbs_smooth_ = 0, const double omega_ = 1.0)
      : QFTAction(lattice_, fine_lattice_, RenormalisationNone), mass(mass_),
        n_gibbs_smooth(n_gibbs_smooth_), omega(omega_), normal_dist(0.0, 1.0) {
    if (lattice->getMt_lat() != lattice->getMx_lat()) {
      mpi_parallel::cerr << "ERROR: Lattice has to be squared for GFF action "
                         << std::endl;
  ...
  virtual unsigned int sample_size() const { return lattice->getNvertices(); };
  ...
  virtual std::shared_ptr<Action> coarse_action() {
    ...
    std::shared_ptr<Action> new_action =
        std::make_shared<GFFAction>(coarse_lattice, lattice, mass, 2, 1.0);
    return new_action;
  };
