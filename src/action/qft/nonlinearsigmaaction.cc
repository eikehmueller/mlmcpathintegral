...
const double NonlinearSigmaAction::evaluate(
  ...
  // Factor 1/2 to avoid overcounting (each edge of the lattice only contributes
  // once)
  return -0.5 * beta * S;
}

/* local heat bath update */
void NonlinearSigmaAction::heatbath_update(
    std::shared_ptr<SampleState> phi_state, const unsigned int ell) {
  // temporary vectors
  ...
  case (0):
    Delta_n_perp[0] = 0.0;
    Delta_n_perp[1] = -Delta_n[2] * rho_inv;
    Delta_n_perp[2] = +Delta_n[1] * rho_inv;
    break;
  case (1):
    Delta_n_perp[0] = -Delta_n[2] * rho_inv;
    Delta_n_perp[1] = 0.0;
    Delta_n_perp[2] = +Delta_n[0] * rho_inv;
    break;
  case (2):
    Delta_n_perp[0] = +Delta_n[1] * rho_inv;
    Delta_n_perp[1] = -Delta_n[0] * rho_inv;
    Delta_n_perp[2] = 0.0;
    break;
  }
  ...
  double theta = atan2(sqrt(sigma_n[0] * sigma_n[0] + sigma_n[1] * sigma_n[1]),
                       sigma_n[2]);
...
void NonlinearSigmaAction::overrelaxation_update(
    std::shared_ptr<SampleState> phi_state, const unsigned int ell) {
  // temporary vectors
  Eigen::Vector3d Delta_n; // sum of nearest neighbour vectors
  double Delta_n_nrm;
...
void NonlinearSigmaAction::force(const std::shared_ptr<SampleState> phi_state,
  ...
  for (unsigned int ell = 0; ell < lattice->getNvertices(); ++ell) {
    ...
    Delta_n = delta_neighbours(phi_state, ell);
    double dS_dtheta =
        -beta * ((Delta_n[0] * cos(phi) + Delta_n[1] * sin(phi)) * cos(theta) -
                 Delta_n[2] * sin(theta));
    double dS_dphi =
        -beta * (-Delta_n[0] * sin(phi) + Delta_n[1] * cos(phi)) * sin(theta);
...
void NonlinearSigmaAction::copy_from_coarse(
    const std::shared_ptr<SampleState> phi_coarse,
    std::shared_ptr<SampleState> phi_state) {
  const std::map<unsigned int, unsigned int> &fine2coarse_map =
      lattice->get_fine2coarse_map();
...
void NonlinearSigmaAction::copy_from_fine(
    const std::shared_ptr<SampleState> phi_fine,
    std::shared_ptr<SampleState> phi_state) {
  const std::map<unsigned int, unsigned int> &fine2coarse_map =
      fine_lattice->get_fine2coarse_map();
  for (auto it = fine2coarse_map.begin(); it != fine2coarse_map.end(); ++it) {
...
void NonlinearSigmaAction::initialise_state(
  ...
  for (unsigned int ell = 0; ell < sample_size() / 2; ++ell) {
    ...
    do {
      ...
      nrm_sq = sigma_n[0] * sigma_n[0] + sigma_n[1] * sigma_n[1] +
               sigma_n[2] * sigma_n[2];
    } while ((nrm_sq > 1.0) or (nrm_sq < 0.01));
    // convert to angles
    double phi = atan2(sigma_n[1], sigma_n[0]);
    double theta = atan2(
        sqrt(sigma_n[0] * sigma_n[0] + sigma_n[1] * sigma_n[1]), sigma_n[2]);
    phi_state->data[2 * ell] = theta;
    phi_state->data[2 * ell + 1] = phi;
  }
}
...
void NonlinearSigmaAction::flip(std::shared_ptr<SampleState> phi_state,
  ...
  double theta = phi_state->data[2 * ell];
  double phi = phi_state->data[2 * ell + 1];
  Eigen::Vector3d sigma = {sin(theta) * cos(phi), sin(theta) * sin(phi),
                           cos(theta)};
