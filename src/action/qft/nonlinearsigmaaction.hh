 ...
 * @brief Header file for action of the O(3) non-linear sigma model in two
 * dimensions
 */
...
public:
  ...
  NonlinearSigmaParameters()
      : Parameters("nonlinearsigma"), beta_(1.0),
 ...
 *
 * where \f$\Delta_n =
 * \sigma_{n+\hat{0}+\sigma_{n-\hat{0}+\sigma_{n+\hat{1}+\sigma_{n-\hat{1}\f$
 * with \f$\sigma_n \in\mathbb{R}^3\f$ and \f$|\sigma_n| = 1\f$
 *
 * This is the lattice version of the continuum action
 *
 * \f[
 *      S_{cont}[\sigma] = \frac{1}{2}\beta\int d^2 \sum_{\mu=0,1} \partial_\mu
 * \sigma(x)\cdot \partial_\mu \sigma(x) \f]
 *
 * The action can be either formulated on a normal, unrotated lattice or on a
 * rotated lattice. In both cases the same Cartesion lattice object is used, but
 * in the lattice case only the points marked in the following diagram are used:
 *
 *
 *   X--+--X--+--X--+--X
 *   !  !  !  !  !  !  !
 ...
 *   X--+--X--+--X--+--X
 *
 * Note that for a rotated lattice the nearest neighbours of a point are defined
 * as the 'diagonal' neighbours. For example, in the above diagram the point 'A'
 * has the four nearest neighbours marked with the letter 'B'.
 *
 */
...
public:
  ...
  NonlinearSigmaAction(const std::shared_ptr<Lattice2D> lattice_,
      ...
      : QFTAction(lattice_, fine_lattice_, renormalisation_),
        ClusterAction(lattice_), beta(beta_), uniform_dist(0., 2. * M_PI),
    ...
    if (not(coarsening_type == CoarsenRotate)) {
      mpi_parallel::cerr
          << "ERROR: invalid coarsening for Non-linear sigma model."
          << std::endl;
  ...
  virtual std::shared_ptr<Action> coarse_action() {
    RenormalisedNonlinearSigmaParameters c_param(lattice, beta,
                                                 renormalisation);
    ...
    new_action = std::make_shared<NonlinearSigmaAction>(
        coarse_lattice, lattice, renormalisation, beta_coarse);
    return new_action;
  };
   ...
   * bath defined by the neighbouring sites. For this, observe that if all spins
   * are kept fixed, except for the spin \f$\sigma_n\f$ at site \f$n\f$, then
   ...
   * \f]
   *
   * Here \f$\omega_n\f$ is the angle between \f$\sigma_n\f$ and the sum of all
   * neighbouring spins \f$\Delta_n\f$. Hence, for the heat-bath update we need
   ...
   *   \f$\pi_n(\omega) \propto \exp\left[2\beta|\Delta_n|
   * \sin^2(\omega/2)\right]\f$
   * 3. find a vector \f$\Delta_n^\perp\f$
   * 4. rotate \f$\sigma_n\f$ around \f$\Delta_n^\perp\f$ by the angle
   * \f$\omega_n\f$
   ...
   *
   * Note that the final two steps leave the action invariant, and are hence
   * also implemented in the overrelaxation step.
   *
   ...
   * overrelaxation.
   *
   * For this, draw an angle \f$\tau_n\f$ uniformly from the interval
   * \f$[0,2\pi[\f$ and rotate \f$\sigma_n\f$ around \f$\Delta_n\f$ by this
   * angle.
   *
   *  @param[inout] phi_state State to update
   *  @param[in] ell index of vertex to update
   ...
   *
   * Since the action can be written as \f$S[\sigma]= - \frac{1}2} \beta \sum_n
   * \sigma_n \cdot \Delta_n\f$, the force at point \f$n\f$is given by \f[
   * \begin{aligned}
   *   \frac{\partial S}/{\partial\theta} &= -\beta\left\[ ((\Delta_n)_0
   * \cos(\phi_n)+(\Delta_n)_1\sin(\phi_n)) \cos(\theta_n)
   *                                -(\Delta_n)_2\sin(\theta_n) \right] \\
   *   \frac{\partial S}/{\partial\phi} &=
   * -\beta\left[-(\Delta_n)_0\sin(\phi_n)+(\Delta_n)_1\cos(\phi_n)\right]
   * \end{aligned}
   * \f]
   *
   *
   ...
   *
   * === Case 2 === we are operating on a rotated lattice (Marked by 'C' and
   * 'F')
   *
   *     current lattice              coarse lattice
   *   C--+--C--+--C--+--C          C-----C-----C-----C
...
private:
  /** @brief Extract and return 3d-vector at lattice vertex with index ell from
   * state
   *
  ...
  Eigen::Vector3d get_sigma(const std::shared_ptr<SampleState> phi_state,
    ...
    Eigen::Vector3d sigma = {sin_theta * cos(phi), sin_theta * sin(phi),
                             cos(theta)};
    return sigma;
  }

  /** @brief Extract 3d-vector at lattice vertex with index ell from sample
   *         state and add it to vector. NB: the only difference to the
   * set_sigma() is that here the vector is inremented, not just set.
   *
   * @param[in] phi_state State \f$\phi\f$
   * @param[in] ell linear index of vertex
  ...
  void add_sigma(const std::shared_ptr<SampleState> phi_state,
                 const unsigned int ell, Eigen::Vector3d &sigma) const {
    double theta = phi_state->data[2 * ell];
    double phi = phi_state->data[2 * ell + 1];
    double sin_theta = sin(theta);
  ...
  /** @brief Extract 3d-vector which is given by the sum of the 3d field vectors
   * of the neighbours of site with linex vertex index ell
   *
   * @param[in] phi_state State \f$\phi\f$
   * @param[in] ell linear index of vertex
  ...
  /** @brief Draw new reflection angle (normal for reflection plan) for cluster
   * sampler */
