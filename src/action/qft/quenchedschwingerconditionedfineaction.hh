...
public:
   ...
   */
  QuenchedSchwingerConditionedFineAction(
      const std::shared_ptr<QuenchedSchwingerAction> action_)
      : action(action_), beta(action->getbeta()), uniform_dist(-M_PI, +M_PI),
        exp_cos_dist(action->getbeta()), bessel_product_dist(NULL),
        approximate_bessel_product_dist(NULL) {
    if (beta > 8.0) {
      // Use Gaussian approximation for larger values of beta?
      approximate_bessel_product_dist =
          std::make_shared<ApproximateBesselProductDistribution>(beta);
    } else {
    ...
    if ((approximate_bessel_product_dist == NULL) and
      ...
      mpi_parallel::cerr << "ERROR: no fill-in distribution defined!"
                         << std::endl;
...
private:
  ...
  /** @brief Approximate probability distribution for drawing vertical interior
   * links */
  mutable std::shared_ptr<ApproximateBesselProductDistribution>
      approximate_bessel_product_dist;
};

/** @class QuenchedSchwingerGaussianConditionedFineAction
 *
 * @brief Conditioned fine action for the quenched Schwinger model with Gaussian
 * approximation
 */
class QuenchedSchwingerGaussianConditionedFineAction
    : public ConditionedFineAction {
public:
  /** @brief Constructor
   *
   * Construct new instance
   ...
   * @param[in] action_ Underlying action class
   */
  QuenchedSchwingerGaussianConditionedFineAction(
      const std::shared_ptr<QuenchedSchwingerAction> action_)
      : action(action_), beta(action->getbeta()), uniform_dist(-M_PI, +M_PI),
        gaussian_fillin_dist(NULL) {
    bool add_gaussian_noise = true;
    gaussian_fillin_dist =
        std::make_shared<GaussianFillinDistribution>(beta, add_gaussian_noise);
    engine.seed(71814151);
  }
 ...
 *
 * @brief Conditioned fine action for the quenched Schwinger model with
 * sem-coarsening
 */
class QuenchedSchwingerSemiConditionedFineAction
    : public ConditionedFineAction {
public:
   ...
   * @param[in] action_ Underlying action class
   */
  QuenchedSchwingerSemiConditionedFineAction(
      const std::shared_ptr<QuenchedSchwingerAction> action_)
      : action(action_), beta(action->getbeta()), uniform_dist(-M_PI, +M_PI),
...
struct QuenchedSchwingerConditionedFineActionFactory
    : public ConditionedFineActionFactory {
  /** @brief Destructor */
  ...
  get(std::shared_ptr<Action> action) {
    std::shared_ptr<QuenchedSchwingerAction> schwinger_action =
    ...
    CoarseningType coarsening_type =
        schwinger_action->get_lattice()->get_coarsening_type();
    if (coarsening_type == CoarsenBoth) {
      return std::make_shared<QuenchedSchwingerConditionedFineAction>(
          schwinger_action);
    } else {
      return std::make_shared<QuenchedSchwingerSemiConditionedFineAction>(
          schwinger_action);
    }
  }
};
