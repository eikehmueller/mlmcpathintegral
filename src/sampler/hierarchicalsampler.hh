...
public:
  ...
  HierarchicalParameters()
      : Parameters("hierarchical"), n_max_level_(2),
  ...
  int readFile(const std::string filename) {
    ...
    if (!readSuccess) {
      ...
      if (sampler_str == "HMC") {
        ...
        mpi_parallel::cerr << "        allowed values are \'HMC\', "
                              "\'heatbath\', \'cluster\', \'exact\'"
                           << std::endl;
 ...
 *
 * Implementation of hierarchical sampling. Generate samples by successively
...
public:
   ...
   * @param[in] coarse_sampler_factory Factory for generating coarse level
  sampler
   * @param[in] conditioned_fine_action_factory factory for generating
  conditioned fine actions
...
public:
  ...
  virtual std::shared_ptr<Sampler> get(std::shared_ptr<Action> action) {
    return std::make_shared<HierarchicalSampler>(
        action, coarse_sampler_factory, conditioned_fine_action_factory,
        param_hierarchical);
  }
