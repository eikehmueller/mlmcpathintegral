...
/* Constructor */
QuenchedSchwingerClusterSampler::QuenchedSchwingerClusterSampler(
    const std::shared_ptr<QuenchedSchwingerAction> action_,
    const ClusterParameters cluster_param)
    : Sampler(), action(action_), engine(723851),
  ...
  std::shared_ptr<Lattice1D> lattice1d =
      std::make_shared<Lattice1D>(ndof_cluster, 1.0);
  rotor_action = std::make_shared<RotorAction>(lattice1d, RenormalisationNone,
                                               beta * lattice1d->geta_lat());
  cluster_sampler =
      std::make_shared<ClusterSampler>(rotor_action, cluster_param);
  rotor_action->initialise_state(psi_cluster_state);
  // Measure cost per sample
  std::shared_ptr<SampleState> phi_state_tmp =
      std::make_shared<SampleState>(ndof);
  Timer timer_meas;
...
/** Draw next sample */
void QuenchedSchwingerClusterSampler::draw(
  ...
  for (int i = 0; i < Mt_lat; ++i) {
    for (int j = 0; j < Mx_lat; ++j) {
      ...
      phi_state->data[lattice2d->link_cart2lin(i - 1, j, 0)] = mod_2pi(
          phi_state->data[lattice2d->link_cart2lin(i - 1, j, 0)] - theta);
      phi_state->data[lattice2d->link_cart2lin(i, j, 1)] =
          mod_2pi(phi_state->data[lattice2d->link_cart2lin(i, j, 1)] + theta);
      phi_state->data[lattice2d->link_cart2lin(i, j - 1, 1)] = mod_2pi(
          phi_state->data[lattice2d->link_cart2lin(i, j - 1, 1)] - theta);
    }
  }
  accept = true;
  n_total_samples++;
