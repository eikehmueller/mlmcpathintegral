...
public:
    ...
    unsigned int n_sweep_heatbath() const {
        return n_sweep_heatbath_;
    }

    /** @brief Return number of overrelaxation sweeps */
    unsigned int n_sweep_overrelax() const {
        return n_sweep_overrelax_;
    }
 ...
 * Generate states by drawing from the local action, conditioned on the neighbouring sites. For this,
 * pass over the unknowns n_{sweep,overrelax} times and apply the overrelaxation method at each site.
 * Following this, carry out n_{sweep,heatbath} sweeps over the unknownes, and locally upate each site
 * with a heat bath step.
 *
 * Sweeps over the unknowns can be carried out lexicographically or in random order.
 *
 * References:
 *  * Creutz, M., 1987. "Monte Carlo algorithms for lattice gauge theory" (No. BNL-39747). Brookhaven National Lab.
 *  * Brown, F.R. and Woch, T.J., 1987. "Overrelaxed heat-bath and Metropolis algorithms for accelerating pure gauge
 *      Monte Carlo calculations." Physical Review Letters, 58(23), p.2394.
 *  * Degrand, T.A. and DeTar, C., 2006. "Lattice methods for quantum chromodynamics." World Scientific (Chapter 7)
 */
...
public:
     ...
     */
    OverrelaxedHeatBathSampler(const std::shared_ptr<Action> action_,
...
protected:
    ...
    /** @brief Random number engine used for shuffling (if unknowns are traversed in random order) */
...
public:
     ...
     */
    OverrelaxedHeatBathSamplerFactory(const OverrelaxedHeatBathParameters param_heatbath_) :
