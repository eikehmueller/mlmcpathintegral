...
bool HMCSampler::single_step() {
  ...
  double deltaS =
      action->evaluate(phi_state_trial) - action->evaluate(phi_state_cur);
...
void HMCSampler::autotune_stepsize(const double p_accept_target) {
  ...
  mpi_parallel::cout
      << " Auto-tuning HMC step size to achieve acceptance rate of "
      << p_accept_target << " ..." << std::endl;
  ...
  for (unsigned int k = 0; k < 100; ++k) {
    ...
    if (fabs(p_accept() - p_accept_target) < tolerance)
      converged = true;
  }
  if (converged) {
    ...
    mpi_parallel::cout << "  FAILED to tune, reverting to " << dt_hmc
                       << std::endl;
  }
