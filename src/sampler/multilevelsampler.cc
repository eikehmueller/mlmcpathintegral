...
MultilevelSampler::MultilevelSampler(
    ...
    : Sampler(), n_level(param_hierarchical.n_max_level() -
                         fine_action->get_coarsening_level()),
      t_indep(n_level, 0.0), n_indep(n_level, 0), t_sampler(n_level, 0),
  ...
  for (unsigned int level = 0; level < n_level; ++level) {
    ...
    stats_sampler.push_back(std::make_shared<Statistics>(
        stats_sampler_label.str(), n_autocorr_window));
  }
  std::shared_ptr<SampleState> meas_state =
...
void MultilevelSampler::draw(std::shared_ptr<SampleState> phi_state) {
  ...
  do {
    ...
    if (t_sampler[level] >= ceil(stats_sampler[level]->tau_int())) {
      ...
      t_indep[level] = (n_indep[level] * t_indep[level] + t_sampler[level]) /
                       (1.0 + n_indep[level]);
...
void MultilevelSampler::show_stats() {
  ...
  mpi_parallel::cout << "  cost per sample = " << cost_per_sample() << " mu s"
                     << std::endl
                     << std::endl;
  for (unsigned int ell = 0; ell < n_level; ++ell) {
    ...
    mpi_parallel::cout << "    average spacing between sample " << t_indep[ell]
                       << std::endl;
