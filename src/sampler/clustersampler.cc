...
ClusterSampler::ClusterSampler(const std::shared_ptr<ClusterAction> action_,
    ...
    : Sampler(), action(action_), lattice(action->get_generic_lattice()),
      n_vertices(lattice->getNvertices()), n_burnin(cluster_param.n_burnin()),
      n_updates(cluster_param.n_updates()), uniform_dist(0.0, 1.0),
...
void ClusterSampler::single_cluster_update() {
  ...
  while (not active_vertices.empty()) {
    ...
    for (auto it = neighbour_vertices[ell].begin();
         it != neighbour_vertices[ell].end(); ++it) {
