...
Due to the strong coupling of QCD at large distances, only lattice calculations allow first-principles predictions. Using Feynman's Path Integral technique \cite{Feynman2010}, QFTs can be be formulated as statistical systems in $d=4$ space-time dimensions. Predictions of physical quantities are then calculated by generating a set of samples with Markov-Chain Monte Carlo Methods and evaluating the relevant quantity of interest (such as a two-point correlation function, which allows the extraction of particle energies) on those samples. To obtain physically meaningful results, calculations need to be carried out for increasingly small lattice spacings $a$ and extrapolated to the continuum limit $a\rightarrow 0$. There are two key issues which make the accurate calculation challenging:
\begin{figure}
  \begin{center}
...
\item Since the number $M$ of lattice sites on a grid with spacing $a$ is $M \propto a^{-d}$, the cost to generate one sample grows as $\mathcal{O}(a^{-d})$. This already assumes that optimal algorithms are used when, for example, inverting large sparse matrices to calculate Fermion propagators. While most theories are formulated in $d=3+1=4$ spacetime dimensions $d=4$, in some case (such as Domain Wall Fermions \cite{Kaplan1992}) the dimension of the system is even $d=5$.
\end{itemize}
...
Feynman's path integral \cite{Feynman2010} allows the calculation of the transition amplitude for starting at the point $x_i$ at time $t=0$ and ending at point $x_f$ at time $t=T$ as a sum over all possible paths $x(t)$ with $x(0)=x_i$,  $x(T)=x_f$, as shown in Fig. \ref{fig:path_integral} (left)\footnote{The figure is slightly misleading since the paths are continuous but not necessarily smooth (differentiable).}. This sum is denoted by $\int \mathcal{D}x(t)$. The crucial observation, exploited for example in \cite{Creutz1981}, is that by formulating the theory in imaginary time, the highly-oscillatory integrand is replaced by a real exponential and the path integral is equivalent to a statistical system of temperature\footnote{In the ``zero-temperature'' limit quantum fluctuations disappear and only the classical trajectory which minimises the action in Eq. (\ref{eqn:action}) contributes to the path integral.} proportional to Planck's constant $\hbar$.  The transition amplitude is given by
\begin{equation}
...
\end{equation}
For example, the two-point correlation function $A=x(0)x(t)$ allows the extration of the system energies $E_n$ for $t\gg 1$ since it can be shown that\footnote{Strictly speaking only true if $\langle x(t)\rangle=0$, which is the case for symmetric potentials.}
\begin{equation}
...
\end{equation}
Here $a$ is the lattice spacing. The quantity $\vec{X}$ is the state vector of the system. The path integral is the sum over all states $X$ with an appropriate exponential weighting $e^{-S[X]}$.
On the lattice the infinite dimensional integral in Eq. (\ref{eqn:expectation_A}) becomes the $M$-dimensional integral
\begin{equation}
...
\begin{equation}
  ...
  \frac{1}{N} \sum_{n=n_0+1}^{N+n_0} X_0^n X_j^n = \langle x(t) x(0)\rangle + \text{(bias error)} + \text{(sampling error)}
\end{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Defining the following \textit{generating functional}
\begin{equation}
  Z[J] = \int \mathcal{D}x(t)\;\exp\left[-S[x(t)]+\int_0^T J(t)x(t)\;dt \right]\label{eqn:generating_functional_cont}
\end{equation}
allows the expression of expectation values as
\begin{equation}
\langle A[x(t)] \rangle = (Z[0])^{-1}A\left(\frac{\delta}{\delta J(t)}\right) Z[J]_{J=0}
\end{equation}
For example, if $A[x(t)]=x(t_1)x(t_2)$ then
\begin{equation}
\langle x(t_1)x(t_2)\rangle = (Z[0])^{-1} \frac{\delta}{\delta J(t_1)}
\frac{\delta}{\delta J(t_2)} Z[J]_{J=0}
\end{equation}
...
\begin{equation}
  \Gamma_c^{(n)} = \frac{\delta}{\delta J(x_1)}\dots\frac{\delta}{\delta J(x_n)} \log\left(Z[J]\right)_{J=0}.
\end{equation}}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
For a harmonic oscillator with $V(x) = \frac{1}{2}\mu^2 x^2$ the 
action can be written as
\begin{equation}
  S[X] = \sum_{j=0}^{M} a \left( \frac{1}{2}m_0 \frac{(X_{j+1}-X_j)^2}{a^2}+\frac{1}{2}\mu^2 X_j^2\right) := \frac{1}{2}X^T G X\label{eqn:ho_action}
\end{equation}
...
\begin{equation}
  G = \begin{pmatrix}
    a\mu^2 + 2m_0/a & -m_0/a & & \dots & & -m_0/a\\
    -m_0/a & a\mu^2 + 2m_0/a & -m_0/a & &\\
    & -m_0/a & a\mu^2 + 2m_0/a & -m_0/a & & \vdots\\
    \vdots & & & \ddots\\
    & & & -m_0/a & a\mu^2 + 2m_0/a & -m_0/a\\
        -m_0/a & & \dots & & -m_0/a & a\mu^2 + 2m_0/a\\
...
\begin{equation}
  \langle X^2\rangle = \langle X_j^2 \rangle = \left(G^{-1}\right)_{jj} = \frac{1}{M}\operatorname{trace}\left(G^{-1}\right) = \sum_{k=0}^{N-1} \left(\mu^2 + \frac{4m_0\sin^2(ka/2)}{a^2}\right)^{-1}
\end{equation}
...
\begin{xalignat}{2}
m_0^{(c)} &= m_0\left(1+\frac{a^2\mu^2}{2m_0}\right)^{-1}, &
\left(\mu^{(c)}\right)^2 &= \mu^2 \left(1+\frac{a^2\mu^2}{4m_0}\right)\left(1+\frac{a^2\mu^2}{2m_0}\right)^{-1}
\end{xalignat}
Hence, the coarse level action also has the same structure as in Eq. (\ref{eqn:ho_action}) and can be written as 
\begin{equation}
  S^{(c)}[X^{(c)}] = \sum_{j=0}^{M/2} 2a \left( \frac{1}{2}m^{(c)}_0 \frac{(X^{(c)}_{j+1}-X^{(c)}_j)^2}{(2a)^2}+\frac{1}{2}\left(\mu^{(c)}\right)^2 \left(X^{(c)}_j\right)^2\right)
\end{equation}
In Fig. \ref{fig:harmonic_oscillator_flow} we show the evolution of the coupling constants as a function of $a$, where we've chosen $m_0(a^*)=0.01$ and $\mu^2(a^*) = 1.0$ and $a^*\ll 1$ is the grid spacing at which we want to carry out our simulation ($m_0$ was chosen small to see the running of the coupling constants for $a<1$). Note that for $a\sim 1$ taking the naive action with $m_0=m_0(a^*)$, $\mu^2=\mu^2(a^*)$ is likely to lead to bad results.
\begin{figure}
\begin{center}
