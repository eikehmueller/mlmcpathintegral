...
void MonteCarloMultiLevel::evaluate() {
  ...
  int n_min_samples = 100; // Minimal number of samples
  ...
  int level = n_level-1;
  double sum_V_ell_over_h_ell;
  std::vector<double> V_ell(n_level,0.0);
  do {
    double h_ell = action[level]->geta_lat();
    double qoi_fine;
    double qoi_Y;
    if (level == (n_level-1)) {
       ...
       * algorithm only every proceed to the next finer level if this is the
       * case
       */
    ...
    // calculate the estimator and 
    // down to the next-finer level
    if ( (stats[level]->samples() > n_min_samples) and
