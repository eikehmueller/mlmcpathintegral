 ...
 * two subgroup elements are \f$h_1(x)=x\f$ and \f$h_2(x)=2\overline{x}-x\f$.
 */
...
public:
   ...
   *
   * The energy of the link is given by
   * \f[
   * E_{\ell} = \frac{I}{a}\left(1-\cos(x_- - x_+)\right)
   * \f]
   *
   * @param[in] x_m Value of \f$x^{(\ell)}_- = x_i\f$
   * @param[in] x_p Value of \f$x^{(\ell)}_+ = x_{i+1}\f$
   */
  virtual double link_E(const double x_m, const double x_p) const {
    return m0/a_lat*(1.-cos(x_m-x_p));
  };
   ...
   * @param[in] x_p Value of \f$x^{(\ell)}_+ = x_{i+1}\f$
   */
  virtual double link_Q(const double x_m, const double x_p) const {
    double E_1 = link_E(x_m,x_p);
    double E_2 = link_E(x_m,2.*xbar-x_p);
    return std::max(E_1,E_2);
  }

  /** @brief Set subgroup \f$H\f$ for the next step of the cluster algorithm
   */
  virtual void new_subgroup() const {
    xbar = uniform_dist(engine);
  }

  /** @brief Return element \f$h\in H\f$ of currently set subgroup 
   */
  virtual void new_subgroup_element() const {
    cur_h = (uniform_dist(engine)>0.0);
  };
   ...
   */
  virtual double apply_current_subgroup_element(const double x) const {
    if (cur_h) {
      return x;
    } else {
      return mod_2pi(2.*xbar-x);
    }
  }
...
protected:
  ...
  mutable double xbar;
  /** @brief current subgroup element 
   * (true: \f$h = Id\f$, false: \f$h=\text{reflection}\f$)*/
  mutable bool cur_h;
